...
compare_metric_prompt = PromptTemplate(
    input_variables=["player1_name", "player2_name", "metric_name", "metric_value1", "metric_value2"],
    template="""
...
Explain why one might be better than the other in this metric and discuss the implications for the match.
...
metric_meanings = {
    ...
    'transaction_rating': 'Transaction rating evaluates the player’s recent transactions.',
...
class Chatbot:
    def __init__(self):
        ...
        self.player_data = pd.read_csv('mock_data.csv')  # Load your mock data
    ...
    def set_players(self, player1_id, player2_id):
        try:
            ...
            print("Invalid player ID(s). Please enter numeric IDs.")
            return False
    ...
    def process_user_query(self, user_query):
        ...
        if any(keyword in user_query.lower() for keyword in explanation_keywords):
            ...
            action = 'compare'  # Changed from 'get' to 'compare'

        metric_name = intent.replace('_', ' ')
        ...
        if action == 'compare':
            ...
            if metric_value1 is not None and metric_value2 is not None:
                ...
                return intent, f"Data for {metric_name} is not available for comparison."
        elif action == 'explain':
